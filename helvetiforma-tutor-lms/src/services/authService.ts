...
class AuthService {
  ...
  async login(credentials: LoginCredentials & { 
    ...
    try {
      ...
      if (credentials.autoLogin && credentials.wordpressUserId) {
        const response = await fetch('/api/tutor/auth/wordpress-login', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            user_id: credentials.wordpressUserId,
            email: credentials.identifier,
            username: credentials.identifier,
            from: credentials.from || 'wordpress'
          }),
        });
        ...
        if (data.success && data.user && data.token) {
          ...
          this.user = user;
          this.cookies = data.token;

          if (typeof window !== 'undefined') {
            localStorage.setItem('tutor_user_data', JSON.stringify(user));
            localStorage.setItem('wp_auth_cookies', data.token);
          }

          return {
            success: true,
            user: user,
            message: data.message || 'Auto-connexion WordPress réussie'
          };
        }

        return {
          success: false,
          message: data.message || 'Échec de l\'auto-connexion WordPress'
        };
      }
