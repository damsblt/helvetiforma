...
class TutorService {
  ...
  async enrollInCourse(courseId: number, userId?: number): Promise<boolean> {
    try {
      ...
      // Use the auto-enroll API for consistent enrollment handling
      const response = await fetch('/api/tutor/auto-enroll', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          course_id: courseId,
          payment_status: 'completed' // For free courses, consider as completed payment
        }),
      });
      ...
      console.error('Enrollment failed:', response.status, response.statusText);
      return false;
    } catch (error) {
  ...
  async getUserEnrollments(userId?: number): Promise<TutorEnrollment[]> {
    try {
      ...
      const approaches = [
        // Approach 1: Try Tutor LMS specific API first (most reliable)
        async () => {
          const user = authService.getUser();
          const params = new URLSearchParams();
          params.append('user_id', enrollUserId.toString());
          if (user?.email) params.append('user_email', user.email);
          
          const response = await fetch(`/api/tutor/enrollments/tutor-lms?${params.toString()}`);
          
          if (response.ok) {
            const result = await response.json();
            console.log('Enrollments from Tutor LMS:', result);
            return result.data || [];
          }
          console.warn(`Tutor LMS API failed: ${response.status}`);
          return [];
        },

        // Approach 2: Try to sync with WordPress data as fallback
        async () => {
          const response = await fetch(`/api/tutor/enrollments/sync`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (response.ok) {
            const result = await response.json();
            console.log('Enrollments synced from WordPress:', result);
            return result.data || [];
          }
          console.warn(`WordPress sync failed: ${response.status}`);
          return [];
        },

        // Approach 2: Try our custom API endpoint as fallback
        async () => {
