<?php
/**
 * Plugin Name: HelvetiForma Auto Enroll Plugin V8
 * Description: Ultra-simplified plugin for auto-enrollment with fixed Basic auth permission check.
 * Version: 1.0.8
 * Author: Damien Balet
 */
...
class HelvetiForma_Auto_Enroll_Plugin_V8 {
    ...
    private function make_user_tutor_student_minimal($user_id) {
        ...
        error_log("HelvetiForma V8: User {$user_id} set as student (minimal)");
        return true;
    }
    ...
    private function enroll_user_in_course_minimal($user_id, $course_id) {
        ...
        if ($existing > 0) {
            error_log("HelvetiForma V8: User {$user_id} already enrolled in course {$course_id}");
            return true;
        }
        ...
        if ($result) {
            error_log("HelvetiForma V8: User {$user_id} enrolled in course {$course_id}");
            return true;
        }

        error_log("HelvetiForma V8: Failed to enroll user {$user_id} in course {$course_id}");
        return false;
    }
...
new HelvetiForma_Auto_Enroll_Plugin_V8();
