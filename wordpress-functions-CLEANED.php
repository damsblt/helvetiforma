...
// =============================================================================
// ARTICLE PURCHASE OVERLAY (ADDED BACK)
// =============================================================================

add_action('wp_footer', 'add_article_purchase_overlay');

// =============================================================================
// FIX TUTORLMS ENROLLMENT AUTO-APPROVAL
// =============================================================================

// Override TutorLMS enrollment status to auto-approve
add_action('tutor_after_enrolled', 'helvetiforma_auto_approve_enrollment', 10, 2);

function helvetiforma_auto_approve_enrollment($course_id, $user_id) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'tutor_enrolled';
    
    // Update enrollment status to 'completed' (auto-approved)
    $wpdb->update(
        $table_name,
        array(
            'status' => 'completed',
    ...
    error_log("HelvetiForma: Auto-approved enrollment for user #{$user_id} in course #{$course_id}");
}
