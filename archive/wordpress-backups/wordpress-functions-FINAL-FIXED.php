...
// =============================================================================
// WOOCOMMERCE AUTOMATION - HOOKS (WORKING VERSION)
// =============================================================================
...
function handle_post_save($post_id, $post, $update) {
    ...
    error_log("HelvetiForma save_post: Article $post_id - Access: $access_level, Price: $price");
...
function handle_acf_article_save($post_id) {
    // Only process posts (articles), not pages or other post types
    ...
    if (!$post || $post->post_status !== 'publish') {
        error_log("HelvetiForma ACF: Article #{$post_id} - Skipping (not published or not found)");
        return;
    }
    
    // ACF fields should be available now
    $access_level = get_field('access', $post_id);
    $price = get_field('price', $post_id);
    
    error_log("HelvetiForma ACF: Article #{$post_id} - Access: {$access_level}, Price: {$price}");
    
    if ($access_level === 'premium' && $price && $price > 0) {
        error_log("HelvetiForma ACF: Creating/updating WooCommerce product for article #{$post_id}");
        create_or_update_woocommerce_product($post_id, $post, $price);
    } else {
        error_log("HelvetiForma ACF: Deleting WooCommerce product for article #{$post_id} (not premium or no price)");
        delete_woocommerce_product($post_id);
    }
}
